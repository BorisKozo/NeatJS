...
export class Genome {
  ...
  feedForward(inputValues) {
    ...
    //asin new inputs
    for (let i = 0; i < this.inputs; i++) this.nodes[i].outputValue = inputValues[i];
  ...
  fullyConnected() {
    ...
    for (let i = 0; i < this.layers - 1; i++) for (let j = i + 1; j < this.layers; j++) maxConnections += nodesPerLayer[i] * nodesPerLayer[j];
  ...
  getNode(x) {
    ...
    for (let i = 0; i < this.nodes.length; i++) if (this.nodes[i].number == x) return i;
  ...
  draw(width = 400, height = 400, container = 'svgContainer') {
    ...
    var svg = d3.select('body').append('svg').attr('width', width).attr('height', height).attr('id', this.id);
